...
test('a closed door blocks everyone, even with the key in the bag', () => {
  const door = [[10,3],[10,4]]; // lock: spade_key
  const s = { openDoors: [], bag: [{id:'spade_key',n:1}] };
  assert.equal(canPass(s, ...door), false);
});

test('an open door lets everyone through', () => {
  const s = { openDoors: ['10,3|10,4'] };
  assert.equal(canPass(s, [10,3], [10,4]), true);
});
